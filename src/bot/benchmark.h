...
class Benchmark {
   public:
    ...
    static int run(GomokuBot& bot) {
        ...
        // We can't easily hook into private `runMCTS` directly unless we are friend or add public
        // benchmark method to Bot. For clean design, let's add a `runBenchmark(durationMs)` method
        // to GomokuBot.
