...
int analyzePatternString(const std::string& pattern) {
    ...
    static const std::vector<std::string> rushFourPatterns = {
        "OXXXX.", ".XXXXO", "#XXXX.", ".XXXX#"
    };
    
    // Jump four patterns (jump four) - X_XXX, XX_XX, XXX_X
    static const std::vector<std::string> jumpFourPatterns = {
        "X.XXX", "XX.XX", "XXX.X"
    };
    ...
    static const std::vector<std::string> jumpLiveThreePatterns = {
        ".X.XX.", ".XX.X.", ".X.XX", "XX.X.", "X.XX."
    };
    
    // Sleep three patterns (sleep three)
    static const std::vector<std::string> sleepThreePatterns = {
        "OXXX..", "..XXXO", "#XXX..", "..XXX#",
        "OX.XX.", ".XX.XO", "OXX.X.", ".X.XXO",
        "OXXX.", ".XXXO", "#XXX.", ".XXX#"
    };
    
    // Live two patterns (live two)
    static const std::vector<std::string> liveTwoPatterns = {
        ".XX..", "..XX.", ".XX."
    };
    
    // Jump live two patterns (jump live two)
    static const std::vector<std::string> jumpLiveTwoPatterns = {
        ".X.X.", ".X..X.", "..X.X."
    };
...
int countPatternScore(const Board& board, int row, int col, Player player) {
    ...
    for (auto& dir : directions) {
        ...
        if (matchesAny(pattern, liveFourPatterns)) threats.liveFours++;
        else if (matchesAny(pattern, rushFourPatterns)) threats.rushFours++;
        else if (matchesAny(pattern, jumpFourPatterns)) threats.jumpFours++;
        else if (matchesAny(pattern, liveThreePatterns)) threats.liveThrees++;
        else if (matchesAny(pattern, jumpLiveThreePatterns)) threats.jumpLiveThrees++;
    }
