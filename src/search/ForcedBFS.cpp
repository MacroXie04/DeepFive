...
bool isFour(const Board& board, int r, int c, Player p) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Forward
        int k = 1;
        while (board.isInside(r + k*dr[i], c + k*dc[i]) && board.at(r + k*dr[i], c + k*dc[i]) == p) {
            count++;
            k++;
        }
        bool open1 = board.isInside(r + k*dr[i], c + k*dc[i]) && board.isEmpty(r + k*dr[i], c + k*dc[i]);
        
        // Backward
        int m = 1;
        while (board.isInside(r - m*dr[i], c - m*dc[i]) && board.at(r - m*dr[i], c - m*dc[i]) == p) {
            count++;
            m++;
        }
        bool open2 = board.isInside(r - m*dr[i], c - m*dc[i]) && board.isEmpty(r - m*dr[i], c - m*dc[i]);
    ...
    if (current == targetSide) {
        ...
        for (const auto& m : moves) {
            ...
            if (nextBoard.checkWinner() == targetSide) {
                vcfNodePool.push_back(std::make_unique<ForcedNode>(nextBoard, 
                    (current==Player::Black)?Player::White:Player::Black, m, node, depth+1));
                return vcfNodePool.back().get();
            }
            
            // Create Child
            Player nextP = (current==Player::Black)?Player::White:Player::Black;
            vcfNodePool.push_back(std::make_unique<ForcedNode>(nextBoard, nextP, m, node, depth+1));
