...
void SidePanel::setupUI() {
    ...
    auto advanceTimerSetupY = [&](int height) { timerSetupY += height + sidebarSpacing; };
    ...
    auto advanceTimerDisplayY = [&](int height) { timerDisplayY += height + sidebarSpacing; };
...
void SidePanel::updateTurnIndicator(Player currentPlayer, bool isHistoryEmpty, GameState state,
                                    Player winner) {
    if (isHistoryEmpty) {
        txtTurn.color(FL_GREEN);
        txtTurn.labelcolor(FL_BLACK);
...
void SidePanel::updateTimerBlocks(double blackTime, double whiteTime, bool isActive,
                                  Player activePlayer, bool flash) {
    auto setBlock = [&](bobcat::TextBox& block, const char* prefix, double timeValue,
                        bool isThisPlayerActive, bool isBlackPlayer) {
        ...
        if (isActive && isThisPlayerActive) {
            if (isLow) {
                ...
                Fl_Color colorB =
                    isBlackPlayer ? fl_rgb_color(40, 40, 40) : fl_rgb_color(220, 220, 220);
